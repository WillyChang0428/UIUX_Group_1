        ...
        <div v-if="isSelected" class="body-section mt-3 pt-3">
            ...
            <div v-if="savedCards.length > 0" class="saved-cards-list mb-3">
                <div v-for="card in savedCards" :key="card.id"
                    ...
                    :class="{ 'is-selected': selectedCardId === card.id }" @click="selectSavedCard(card.id)">
                    <div class="d-flex align-items-center gap-3">
                        <span class="brand-badge" :class="card.brand.toLowerCase()">
                            {{ card.brand }}
                        </span>
                        <span class="text-white fs-6">{{ card.bankName }} / ****{{ card.last4 }}</span>
                    </div>
                    <i v-if="selectedCardId === card.id" class="fa-solid fa-circle-check text-primary fs-5"></i>
                </div>
            </div>

            <div class="add-card-toggle d-flex align-items-center gap-2 mb-3 cursor-pointer" @click="toggleAddCardForm">
                <i class="fa-solid fs-4 text-secondary"
                    :class="isAddingCard ? 'fa-circle-minus' : 'fa-circle-plus'"></i>
...
const selectSavedCard = (id) => {
    ...
    isAddingCard.value = false; // 選了舊卡就把新增表單收起來
};
...
const handleSaveCard = () => {
    ...
    // 建立新卡片物件 (這裡為示意，固定命名國泰銀-cube卡，實務上可根據 BIN code 判斷)
    const newCard = {
        id: Date.now(), // 產生亂數 ID
    ...
    selectedCardId.value = newCard.id; // 自動選取剛綁好的卡
