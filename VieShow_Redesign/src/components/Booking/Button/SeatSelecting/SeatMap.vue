  ...
  <div class="seat-map" ref="seatMapRef">
    ...
    <div class="seat-map__toolbar" style="position: relative; z-index: 15">
      <button class="seat-map__tool-btn" @click="isPanoramaOpen = true" aria-label="開啟實景環景"
        :style="{ opacity: isPreviewOpen ? 0 : 1, pointerEvents: isPreviewOpen ? 'none' : 'auto', transition: 'opacity 0.2s' }">
        <i class="fa-solid fa-camera fs-4"></i>
      </button>

      <button class="seat-map__tool-btn" @click="togglePreview" :aria-label="isPreviewOpen ? '縮回' : '放大預覽'">
        <i :class="['fa-solid', isPreviewOpen ? 'fa-compress' : 'fa-expand', 'fs-4']"></i>
      </button>
    </div>
      ...
      <div v-if="isPreviewOpen" class="seat-map__preview-overlay">
        ...
        <div class="seat-map__preview-scroll">
          <div class="seat-map__grid seat-map__grid--lg">
            <div v-for="(row, rowIndex) in seatLayout" :key="`lg-${rowIndex}`" class="seat-row">
              <template v-for="(seatCode, colIndex) in row" :key="`lg-${colIndex}`">
                ...
                <div v-else class="seat-wrap">
                  <SeatButton size="lg" :state="getSeatState(seatCode, rowIndex, colIndex)" @click="handleSeatClick(rowIndex, colIndex, seatCode)" />
                  <Transition name="tooltip-pop">
                    <div v-if="tooltip.key === `${rowIndex}-${colIndex}`" class="seat-tooltip seat-tooltip--lg"
                      :class="[ rowIndex < 3 ? 'seat-tooltip--below' : 'seat-tooltip--above', getTooltipClamp(colIndex, row) ]">
    ...
    <div class="seat-map__grid-wrap mb-4" :style="{ opacity: isPreviewOpen ? 0 : 1, pointerEvents: isPreviewOpen ? 'none' : 'auto', transition: 'opacity 0.2s ease' }">
        ...
        <div v-for="(row, rowIndex) in seatLayout" :key="rowIndex" class="seat-row">
          <template v-for="(seatCode, colIndex) in row" :key="colIndex">
            ...
            <div v-else class="seat-wrap" :data-col-index="colIndex" :data-row-index="rowIndex">
              <SeatButton size="md" :state="getSeatState(seatCode, rowIndex, colIndex)" @click="handleSeatClick(rowIndex, colIndex, seatCode)" />
              <Transition name="tooltip-pop">
                <div v-if="tooltip.key === `${rowIndex}-${colIndex}`" class="seat-tooltip"
                  :class="[ rowIndex < 3 ? 'seat-tooltip--below' : 'seat-tooltip--above', getTooltipClamp(colIndex, row) ]">
                  <p class="seat-tooltip__text">{{ ROW_LABELS[rowIndex] }} 排</p>
                  <p class="seat-tooltip__text">{{ getDisplayCol(row, colIndex) }} 號座位</p>
                  <div class="seat-tooltip__arrow"></div>
    ...
    <div :style="{ opacity: isPreviewOpen ? 0 : 1, pointerEvents: isPreviewOpen ? 'none' : 'auto', transition: 'opacity 0.2s ease' }">
      <SeatLegend />
    </div>

    <div class="text-center mt-4 mb-2" :style="{ opacity: isPreviewOpen ? 0 : 1, transition: 'opacity 0.2s ease' }">
      <p v-if="remainingSeats > 0" class="mb-0 fw-bold text-white">尚需選 {{ remainingSeats }} 個座位</p>
      <p v-else-if="remainingSeats === 0" class="mb-0 fw-bold text-success">座位已選滿，請點擊下一步！</p>
      <p v-else class="mb-0 fw-bold text-danger">您多選了 {{ Math.abs(remainingSeats) }} 個座位，請取消多餘的位子。</p>
    </div>
...
const remainingSeats = computed(() => props.ticketCount - selectedSeats.value.length);
...
.seat-map__grid-wrap { width: 100%; overflow: visible; }
.seat-map__grid { display: flex; flex-direction: column; align-items: center; gap: 5px; }
.seat-row { display: flex; align-items: center; gap: 4px; }

.aisle {
  flex-shrink: 0;
  &--main { width: 20px; }
  &--wheelchair-gap { width: 30px; }
  &--lg {
    &.aisle--main { width: 28px; }
    &.aisle--wheelchair-gap { width: 50px; }
  }
}
...
.seat-tooltip {
  ...
  &--above {
    ...
    .seat-tooltip__arrow {
      top: 100%; border-left: 9px solid transparent; border-right: 9px solid transparent; border-top: 9px solid $vieshow-primary;
      &::after { content: ""; position: absolute; top: -11px; left: 50%; transform: translateX(-50%); border-left: 7px solid transparent; border-right: 7px solid transparent; border-top: 8px solid #0d2a4a; }
    }
  }
  ...
  &--below {
    ...
    .seat-tooltip__arrow {
      bottom: 100%; border-left: 9px solid transparent; border-right: 9px solid transparent; border-bottom: 9px solid $vieshow-primary;
      &::after { content: ""; position: absolute; bottom: -11px; left: 50%; transform: translateX(-50%); border-left: 7px solid transparent; border-right: 7px solid transparent; border-bottom: 8px solid #0d2a4a; }
    }
  }

  &--clamp-left { left: 0; transform: translateX(0); .seat-tooltip__arrow { left: 15px; transform: translateX(0); } }
  &--clamp-right { left: auto; right: 0; transform: translateX(0); .seat-tooltip__arrow { left: auto; right: 15px; transform: translateX(0); } }
  &--lg { padding: 8px 24px; border-radius: 12px; border-width: 3px; }
}

.seat-tooltip__text {
  margin: 0; font-size: 20px; font-weight: 700; color: $light; line-height: 1.4;
  .seat-tooltip--lg & { font-size: 24px; }
}

.seat-tooltip__arrow { position: absolute; left: 50%; transform: translateX(-50%); width: 0; height: 0; }

// ── 放大預覽遮罩 ──
.seat-map__preview-overlay {
  position: absolute; inset: 0; z-index: 10; display: flex; flex-direction: column; gap: 8px; padding: 8px; overflow: hidden;
}

.seat-map__preview-scroll {
  overflow: auto; scrollbar-width: thin; scrollbar-color: rgba($white, 0.2) transparent; cursor: grab; flex: 1; -webkit-overflow-scrolling: touch;
  &:active { cursor: grabbing; }
  .seat-map__grid--lg { width: max-content; min-width: 100%; margin: 0 auto; padding: 8px; .seat-row { gap: 6px; } }
}
...
.tooltip-pop-enter-from { opacity: 0; transform: translateX(-50%) translateY(6px); }
