  ...
  <div class="ticket-selector">
    ...
    <section class="ticket-section">
      <h2 class="ticket-section__title">
        其他票種
        <span class="ticket-section__subtitle"
          >＊以下票種無法與一般票種同時購買</span
        >
      </h2>
...
// 💡 新增：深度監聽所有票種陣列，合併後存入 Pinia
watch(
  [generalTickets, bankTickets, otherTickets],
  () => {
    const allTickets = [
      ...generalTickets.value,
      ...bankTickets.value,
    ...
    const selectedTickets = allTickets.filter((t) => t.quantity > 0);
    bookingStore.updateTickets(selectedTickets);
  },
  { deep: true },
);
</script>
